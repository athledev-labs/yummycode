...
function detectPersona(system: string): 'parent' | 'pm' {
  const s = system.toLowerCase();
  if (s.includes('product manager') || s.includes(' pm') || s.includes('curious pm')) return 'pm';
  return 'parent';
}
...
function personaReply(messages: ChatMessage[]): string {
  ...
  const openers = persona === 'pm' ? PM_OPENERS : PARENT_OPENERS;
  const jargonPush = persona === 'pm' ? PM_JARGON_PUSH : PARENT_JARGON_PUSH;
  const probes = persona === 'pm' ? PM_PROBES : PARENT_PROBES;
